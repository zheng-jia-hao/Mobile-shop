<template>
    <div class="shop-header">
      ...
      <div class="header-content">
        <h2 class="content-title">
          ...
          <span class="content-name" @click="isShowShopDetail = true">{{info.name}}</span>
    ...
    <!-- 
      ...
      如果初始化显示报这样的错：
          ...
          或者：Error in render: "TypeError: Cannot read property '0' of undefined"

      都是因为数据是动态的，而模板的渲染是比数据更快到达，所以会报错

      解决方法：

          数据预处理：
              添加v-if 限定数据显示的时间段，没有数据前不进行读取就不会报错了

     -->
    <div class="shop-header-discounts" @click="isShowDiscount = true" v-if="info.supports">
      <div class="discounts-left"   >
        <!-- 根据数据type的不同 定义了一个类名的数组，按对应的下标选择对应的类名 -->
    ...
    <transition name="fade">
      <div class="shop-brief-modal" v-show="isShowShopDetail">
        <div class="brief-modal-content">
          ...
          <div class="mask-footer">
            <span class="iconfont icon-close" @click="isShowShopDetail=false"></span>
          </div>
        </div>
        <div class="brief-modal-cover" @click="isShowShopDetail=false"></div>
      </div>
    </transition>

    <transition name="move">
        <div class="activity-sheet" v-show="isShowDiscount">
      <div class="activity-sheet-content">
        <h2 class="activity-sheet-title">
        优惠活动</h2>

        <ul class="list">
          <li class="activity-item" :class="supportsClass[s.type]" v-for="(s,index) in info.supports" :key="index">
            <span class="content-tag">
              <span class="mini-tag">{{s.name}}</span>
            </span>
            <span class="activity-content">{{s.content}}</span>
          </li>
        </ul>

        <div class="activity-sheet-close">
          <span class="iconfont icon-close" @click="isShowDiscount=false"></span>
        </div>
      </div>
      <div class="activity-sheet-cover" @click="isShowDiscount=false"></div>
        </div>
    </transition>
